        ...
        def apply_trace_defaults(trace):
            ...
            if trace.type == "scatter":
                if "groupby" in options and figure_options["fix_xaxis"]:
                    ...
                    trace_c = x_vals.merge(
                        trace_c, on="x", how="left").fillna(0)
            ...
            if trace.type == "bar":
                ...
                fig.update_xaxes(get_xaxis_layout(
                    options), linecolor="#BCCCDC")
            if trace.type == "box":
                fig.update_xaxes(get_xaxis_layout(
                    options), linecolor="#BCCCDC")
                fig.update_yaxes(
...
def get_complete_xaxis(options):
    start_date = options["start_date"] if (
        "start_date" in options) else "2009-04-01"
    end_date = (
        options["end_date"]
...
def fig_is_empty(fig):
    ...
    if fig.data[0].type in ["histogram", "box"]:
        ...
        return (
            sum(sum(x) if isinstance(x, list)
                else x for x in list(fig.data[0].z)) == 0
        )
